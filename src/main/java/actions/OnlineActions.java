...

{
	...
	public static void SetTextAddress(WebElement element, String textToType, String elementDesc) throws IOException
	{
		logger.info("--------	EnterText wrapper method inside FalconActions class is invoked successfully	--------");
		try
		{
			...
			jse.executeScript("arguments[0].value='"+ textToType+ "';",element);
	...
	public static void waitForElementToBeClickable(WebElement locator,String elementDesc) throws IOException
	{
		...
		{
			//Thread.sleep(1000); //Added on 24-June-2022
			Thread.sleep(2000); //Added on 24-June-2022 Added for slow-down the speed/pace
			logger.info(locator);
			WebDriverWait wait = applyWebDriverWait();
...
{
	Actions actions = new Actions(driver);
	actions.sendKeys(Keys.PAGE_UP).build().perform(); //Page UpSystem.out.println("Scroll up perfomed");Thread.sleep(3000);
}
...
public static void loadingWaitss(WebElement loader) {
...
public static void loadingWait(WebElement locator,String elementDesc) throws IOException
{
	try
	{
		//logger.info(locator);
		WebDriverWait wait = applyWebDriverWait();
		wait.until(ExpectedConditions.visibilityOf(locator)); // wait for loader to appear
